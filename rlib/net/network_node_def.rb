...
class Nodes
    ...
    @nodes.each do |node_name, node_details|
      ...
      node_details[:interfaces].each do |interface|
        @network_routes[interface[:network_name]].each do |gw, routes|
          if gw == interface[:ipv4] # Local interfaces.
          ...
          else # Don't want to record routes that go through us.
  ...
  private def routes_i_have(network_name:, path: [], hop_count: 1)
    if @network_routes_next_hop[network_name] != nil # This network is connected to other networks.
      @network_routes_next_hop[network_name].each do |route| # For each route from this network.
        ...
        next unless route[:network_name] != nil && ! path.include?(route[:node_name])
  ...
  private def path_contains_node_on_this_net(network:, path:)
  ...
  private def route_not_present?(network:, routes:, new_route:)
    break_next = false
  ...
  private def calculate_all_routes
    ...
    @network_routes_next_hop.each do |network_name, _routes|
      ...
      routes_i_have(network_name: network_name, path: [], hop_count: 1) do |route|
        ...
        elsif route_not_present?(network: network_name, routes: @network_routes[network_name][route[:gw]], new_route: route)
          @network_routes[network_name][route[:gw]] << route
        end
      end
      if default_route[:gw] != @default_route && route_not_present?(network: network_name, routes: @network_routes[network_name][default_route[:gw]], new_route: default_route)
        @network_routes[network_name][default_route[:gw]] << default_route
      end
    end
  end
    ...
    @networks_nodes.each do |network_name, nodes|
      ...
      nodes.each do |node_name|
        ...
        @nodes[node_name][:interfaces].each do |i|
          next unless network_name != i[:network_name] && i[:route_hint] != nil
